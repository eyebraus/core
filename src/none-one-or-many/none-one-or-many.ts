 ...
 */
export const NoneOrOne = <T extends Defined>(value?: T): NoneOrOne<T> => (value ? [value] : []);

/**
 * A value that is an array containing a single defined value.
 */
export type One<T extends Defined> = [T];
...
export const OneOrMany = <T extends Defined>(value: T, ...others: T[]): OneOrMany<T> =>
    isMany(others) ? [value, ...others] : isOne(others) ? [value, ...others] : [value];
