...
#### String Utilities
- **padZero**: Pads a number with leading 0s to match a certain length

#### Number Utilities
- **clamp**: Bounds a given number to a defined range
- **modulo**: Performs a modulo (%) operation whose result is guaranteed to be positive
- **randomInteger**: Generates a random integer between 0 and a specified maximum (exclusive)

#### Array Utilities
- **isEquivalent**: Checks whether two different arrays have equivalent contents
- **removeAllInPlace**: Removes all occurrences of an element from an array in-place
- **removeInPlace**: Removes the first occurrence of an element from an array in-place
